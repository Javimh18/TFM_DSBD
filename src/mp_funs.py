...
def extract_right_hand_landmarks(results):
    return np.array([[res.x, res.y, res.z] for res in results.right_hand_landmarks.landmark])
                    # \if results.right_hand_landmarks else np.zeros(HAND_LANDMARKS) 
                    # the people in the signs videos are well captured so there's no need for this
    

def extract_left_hand_landmarks(results):
    return np.array([[res.x, res.y, res.z] for res in results.left_hand_landmarks.landmark])
                    # \if results.left_hand_landmarks 
                    # else np.zeros(HAND_LANDMARKS) 
                    # the people in the signs videos are well captured so there's no need for this

def extract_pose_landmarks(results):
    return np.array([[res.x, res.y, res.z, res.visibility] for res in results.pose_landmarks.landmark])
                    # \if results.pose_landmarks 
                    # else np.zeros(HAND_LANDMARKS) 
                    # the people in the signs videos are well captured so there's no need for this

def extract_facemesh_landmarks(results):
    return np.array([[res.x, res.y, res.z] for res in results.face_landmarks.landmark])
                    # \if results.pose_landmarks 
                    # else np.zeros(HAND_LANDMARKS) 
                    # the people in the signs videos are well captured so there's no need for this
